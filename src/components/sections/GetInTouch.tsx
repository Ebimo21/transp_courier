...
const GetInTouch = ({ImageId}: Props) => {
    const {lead, leadView, p, pView, points, pointsView } = useWhenInView();
    
  return (
    <>
        <div  className=' bg-lightYellow group '>
            <div className='flex flex-wrap gap-5 max-w-[1250px] m-auto'>
                <div  className='basis-full lg:basis-5/12 mx-6 lg:ml-32 my-10 '>
                    <span  className='bg-yellow px-4 py-1 font-bold'>Get In Touch</span>
                    <h3 ref={lead} className={`text-lightBlue text-2xl font-bold  mt-4 opacity-0 ${leadView? "slide-top":""}`}>Proud to Deliver <br />Excellence Every Time</h3>
